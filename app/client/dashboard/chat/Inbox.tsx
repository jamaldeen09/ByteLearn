...
const Inbox = () => {
  ...
  return (
    // <div className="col-span-14 h-[90vh] grid grid-cols-16 gap-1 basic-border">

    //   <AnimatePresence>
    //     <motion.div
    //       className={`${isMobile && isNotifClicked && "hidden"} h-full py-4 rounded-xl overflow-hidden col-span-16 basic-border max-lg:col-span-6 bg-gray-50 flex flex-col gap-4 border border-gray-200`}
    //       initial={{ opacity: 0 }}
    //       animate={{ opacity: 1 }}
    //       transition={{ duration: 0.3 }}
    //       exit={isNotifClicked ? { x: "-100vw" } : {}}
    //       key="sidebar"
    //     >
    //       {/* Search Header */}
    //       <div className="px-4">
    ...
    //        key="main-notif-content"
    //       className={`border-4 border-purple-500
    //       w-full h-full hidden max-lg:block max-lg:col-span-10"
    //       ${isNotifClicked && isMobile && ""}`}
    //       // initial={isNotifClicked && isMobile ? { x: "100vw" }: {x: 0}}
    //       // animate={isNotifClicked && isMobile ? {x: 0} : {}}
    //     >
